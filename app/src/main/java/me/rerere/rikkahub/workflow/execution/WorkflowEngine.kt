    ...
    private fun triggerRuntimeCheck(trigger: me.rerere.rikkahub.workflow.model.TriggerSpec): String? {
        ...
        return when (trigger) {
            ...
            is me.rerere.rikkahub.workflow.model.TriggerSpec.GeofenceExit -> {
                ...
                when {
                    !fineGranted -> "geofence_unavailable: ACCESS_FINE_LOCATION not granted — open Settings → Apps → Moataz Alaqami → Permissions → Location and pick Allow all the time"
                    !bgGranted -> "geofence_unavailable: ACCESS_BACKGROUND_LOCATION not granted — open Settings → Apps → Moataz Alaqami → Permissions → Location and pick Allow all the time"
                    else -> null
                }
            }
            is me.rerere.rikkahub.workflow.model.TriggerSpec.NotificationReceived -> {
                if (!me.rerere.rikkahub.data.ai.tools.local.NotificationListenerHandle.isBound()) {
                    "notification_listener_not_enabled: enable the Moataz Alaqami notification listener in Settings → Apps → Special access → Notification access"
                } else null
            }
            ...
            is me.rerere.rikkahub.workflow.model.TriggerSpec.AppClosed -> {
                if (!me.rerere.rikkahub.data.ai.tools.local.AccessibilityServiceHandle.isRunning()) {
                    "accessibility_not_enabled: enable the Moataz Alaqami accessibility service in Settings → Accessibility (required for app_launched / app_closed triggers)"
                } else null
            }

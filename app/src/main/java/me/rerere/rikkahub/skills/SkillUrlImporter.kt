...
class SkillUrlImporter(
    ...
    private fun transcodeFromHermes(raw: String, sourceUrl: String, override: String?): String? {
        ...
        val body = buildString {
            ...
            appendLine("## Source")
            appendLine()
            appendLine("Imported from Hermes skill at $sourceUrl. Tool names were not transcoded — Hermes references its own tool surface, which doesn't always map 1:1 to Moataz Alaqami's. Edit any tool references manually if the skill misbehaves.")
        }
